...
> TextGO 是一个跨平台的文本处理工具，能够自动识别文本类型并执行自定义动作。可通过键盘快捷键、鼠标双击或拖拽选中等方式触发，支持立即执行动作或弹出工具栏供交互选择。
...
1. **多种触发方式**：支持键盘快捷键、鼠标双击或拖拽选中触发，每种方式可独立配置规则
2. **灵活执行模式**：支持立即执行或工具栏交互模式，自由切换以适应不同场景
3. **开箱即用**：内置丰富的文本类型和处理动作，简单配置即可使用
4. **高度可定制**：可通过正则、机器学习模型、脚本或接入本地 AI 扩展识别和处理能力
